...
namespace ini
{
	...
	// ArYa: Modified to encode/decode char as a numerical value rather than character
	template<>
	struct Convert<char>
	{
		void decode(const std::string& value, char& result)
		{
			...
			if (!strToLong(value, tmp))
				throw std::invalid_argument("field is not a char");
	...
	// ArYa: Modified to encode/decode unsigned char as a numerical value rather than character
	template<>
