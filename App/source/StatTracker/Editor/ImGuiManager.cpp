...
{
  m_imguiInit = ImGui::SFML::Init(*game::getWindowManager()->getWindow(), false);

  if (m_imguiInit)
  {
    ImGuiIO& io = ImGui::GetIO();

    const std::string path = game::getResourcesDir() + "/Inter-Regular.ttf";
...
{
  sf::Time time = game::getTickManager()->getTime();
...
void ImGuiManager::onRender()
{
  if (m_imguiInit)
  {
    ImGui::SFML::Render(*game::getWindowManager()->getWindow());
  }
}